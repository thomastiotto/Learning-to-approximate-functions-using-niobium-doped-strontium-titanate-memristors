...
with model:
    ...
    pre = nengo.Ensemble( 400, dimensions=4 )
    nengo.Connection( inp, pre )
    
    # -- post population
    post = nengo.Ensemble( 400, dimensions=1 )
    ...
    product_a = nengo.networks.Product( n_neurons=100, dimensions=1, input_magnitude=1 )
    nengo.Connection( inp[ 0 ], product_a.input_a )
    nengo.Connection( inp[ 1 ], product_a.input_b )
    product_b = nengo.networks.Product( n_neurons=100, dimensions=1, input_magnitude=1 )
    nengo.Connection( inp[ 2 ], product_b.input_a )
    nengo.Connection( inp[ 3 ], product_b.input_b )
    combined_product = nengo.Ensemble( 400, dimensions=1 )
    nengo.Connection( product_a.output, combined_product )
    nengo.Connection( product_b.output, combined_product )
    
    # -- error population
    error = nengo.Ensemble( 100, dimensions=1 )
    nengo.Connection( post, error )
    nengo.Connection( combined_product, error, transform=-1 )
    ...
    # -- probes
    product_probe = nengo.Probe( product_a.output, synapse=0.01 )
